    ...
    def export_to_excel(self):
        ...
        for date, group in df.groupby("Дата"):
            ...
            for tax_char, tax_data in sorted_taxes:
                ...
                # Додаємо до гранд-підсумку
                if tax_char not in grand_taxes:
                    grand_taxes[tax_char] = {"turnover": 0.0, "vat": 0.0}
                grand_taxes[tax_char]["turnover"] += turnover
                grand_taxes[tax_char]["vat"] += vat_amount

                if turnover != 0:
                    output_rows.append(["", "", "", f"Обіг Група {tax_char} (сума чеків)", f"{turnover:.2f}", ""])
                if vat_amount != 0:
                    output_rows.append(["", "", "", f"Податок Група {tax_char} (ПДВ/Акциз)", f"{vat_amount:.2f}", ""])
        ...
        for tax_char, tax_data in sorted(grand_taxes.items()):
            ...
            if tax_data["vat"] != 0:
                output_rows.append(["", "", "", f"ЗАГАЛЬНИЙ ПОДАТОК ГРУПА {tax_char}", f"{tax_data['vat']:.2f}", ""])
